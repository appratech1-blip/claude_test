...
- **3D hero accent**: `Hero3D.tsx` is a lazy-loaded chunk rendered beside the hero headline, gated to desktop + motion-enabled visitors only (see `useIsDesktop` and `useReducedMotion` in `Hero.tsx`); mobile and reduced-motion visitors get the `HeroParticles` canvas fallback instead. Budget: ~250KB gzipped for the lazy hero chunk (R3F stack), desktop + motion-enabled only.
